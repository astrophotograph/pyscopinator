...
class MountCapabilities(BaseModel):
    ...
    can_set_tracking: bool = Field(
        default=True, description="Can enable/disable tracking"
    )
    can_set_tracking_rate: bool = Field(
        default=False, description="Can set tracking rate"
    )
...
class CameraCapabilities(BaseModel):
    ...
    can_stop_exposure: bool = Field(
        default=False, description="Can stop exposure early"
    )
    ...
    has_filter_wheel: bool = Field(
        default=False, description="Has integrated filter wheel"
    )
    ...
    pixel_size_x: float = Field(
        default=0.0, ge=0, description="Pixel width in micrometers"
    )
    pixel_size_y: float = Field(
        default=0.0, ge=0, description="Pixel height in micrometers"
    )
    min_exposure: float = Field(
        default=0.0, ge=0, description="Minimum exposure in seconds"
    )
    max_exposure: float = Field(
        default=3600.0, gt=0, description="Maximum exposure in seconds"
    )
    supported_gains: list[int] = Field(
        default_factory=list, description="Available gain values"
    )
...
class FocuserCapabilities(BaseModel):
    ...
    can_absolute: bool = Field(
        default=True, description="Supports absolute positioning"
    )
    ...
    has_temp_compensation: bool = Field(
        default=False, description="Supports temp compensation"
    )
...
class FilterWheelCapabilities(BaseModel):
    ...
    filter_names: list[str] = Field(
        default_factory=list, description="Filter names by position"
    )
