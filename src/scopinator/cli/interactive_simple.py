    ...
    def complete(self, text, state):
        ...
        if state == 0:
            ...
            if " " not in before_cursor.strip():
                ...
                if text:
                    self.matches = [
                        cmd for cmd in self.commands if cmd.startswith(text.lower())
                    ]
                else:
                ...
                if cmd_name in self.cli_group.commands:
                    ...
                    if text:
                        self.matches = [
                            cmd for cmd in cmd_list if cmd.startswith(text.lower())
                        ]
                    else:
...
def run_interactive_mode(cli_group, ctx):
    ...
    if readline.get_completer():
        if is_macos and is_libedit:
            click.echo(
                "📝 Note: On macOS, tab completion may require pressing Tab twice."
            )
        click.echo("")
    else:
...
def execute_command(cli_group, ctx, command):
    ...
    if cmd and cmd_name != "interactive":
        ...
        if cmd_name != "interactive":
            ...
            similar = [
                c
                for c in cli_group.commands.keys()
