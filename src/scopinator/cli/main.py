...
@click.option(
    "--debug",
    is_flag=True,
    help="Enable debug logging (shows detailed connection info)",
)
@click.option(
    "--trace",
    is_flag=True,
    help="Enable trace logging (most verbose, shows all internal operations)",
)
@click.option(
    "--quiet", is_flag=True, help="Reduce logging to warnings and errors only"
)
@click.option(
    "--log-level",
    type=click.Choice(
...
@click.option(
    "--protocol",
    "-P",
    type=click.Choice(["seestar", "alpaca", "indi", "auto"]),
    default="auto",
    help="Protocol to use (default: auto-detect)",
)
...
from scopinator.cli.repl_enhanced import register_enhanced_repl  # noqa: E402

register_enhanced_repl(cli)

# Register profile management commands
from scopinator.cli.commands.profile import profile as profile_group  # noqa: E402
...
@click.option(
    "--port", "-p", type=int, help="Port number (default depends on protocol)"
)
@click.option(
    "--timeout", "-t", default=5.0, help="Discovery timeout in seconds (default: 5)"
)
@click.pass_context
def discover(ctx, host, port, timeout):
    ...
    """
    from scopinator.cli.protocol_helper import DEFAULT_PORTS
    import time
    ...
    async def run_discovery():
        if host:
                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        ...
                        if devices:
                            ...
                            click.echo(
                                f"✅ Found {total} Alpaca device(s) at {host}:{final_port}"
                            )
                            result = []
                            ...
                            click.echo(
                                f"❌ No Alpaca devices found at {host}:{final_port}"
                            )
                            return []
                except Exception as e:
            ...
            if protocol == "seestar":
                ...
                click.echo(
                    f"🔍 Searching for Seestar telescopes on the network (timeout: {timeout}s)..."
                )
                ...
                return [
                    {"type": "seestar", "host": ip, "port": p} for ip, p in telescopes
                ]
                ...
                click.echo(
                    f"🔍 Searching for Alpaca servers on the network (timeout: {timeout}s)..."
                )
                ...
                async with aiohttp.ClientSession() as session:
                    for server in servers:
                        ...
                        click.echo(
                            f"   Found server at {server_host}:{alpaca_port}, querying devices..."
                        )

                        try:
                            discovery = AlpacaDiscovery(
                                session,
                                f"http://{server_host}:{alpaca_port}",
                                timeout=5.0,
                            )
                            devices = await discovery.get_configured_devices()
                            for device_type, device_list in devices.items():
                            ...
                            click.echo(
                                f"   ⚠️ Could not query devices on {server_host}:{alpaca_port}: {e}"
                            )
                ...
                click.echo("   Please specify --host")
                return []
    ...
    async def test_connection():
        try:
            client = await asyncio.wait_for(
                create_client(protocol, host, final_port), timeout=timeout
            )
            ...
            try:
                ...
                if status.get("ra") is not None and status.get("dec") is not None:
                    click.echo(
                        f"   Position: RA {status['ra']:.4f}, Dec {status['dec']:.4f}"
                    )
            except Exception:
...
def disconnect():
    ...
    """
    from scopinator.cli.connection_state import (
    ...
    if state:
        clear_connection_state()
        click.echo(
            f"Cleared connection to {state.get('host')}:{state.get('port')} ({state.get('protocol')})"
        )
    else:
...
@cli.command()
@click.option(
    "--host", "-h", help="Telescope IP (uses saved connection if not provided)"
)
...
def status(ctx, host, port, detailed):
    ...
    protocol, resolved_host, resolved_port = get_protocol_and_connection(
        ctx, host, port
    )
    ...
    async def get_status():
        try:
            ...
            if protocol == "seestar" and detailed and client._seestar_client:
                ...
                if device_response and device_response.result:
                    ...
                    if "pi_status" in device_state:
                        ...
                        click.echo(
                            f"Charging: {'Yes' if pi.get('charge_online') else 'No'}"
                        )
                        click.echo(f"Temperature: {pi.get('temp', 'N/A')}°C")
                        click.echo(f"Battery Temp: {pi.get('battery_temp', 'N/A')}°C")
                        click.echo(
                            f"Over Temperature: {'Yes' if pi.get('is_overtemp') else 'No'}"
                        )
                        click.echo()
                    ...
                    if "storage" in device_state:
                        ...
                        click.echo(
                            f"Current Storage: {storage.get('cur_storage', 'N/A')}"
                        )
                        if "storage_volume" in storage:
                            for vol in storage["storage_volume"]:
                                click.echo(
                                    f"  {vol.get('name', 'N/A')}: {vol.get('freeMB', 0):,} MB free / {vol.get('totalMB', 0):,} MB total ({vol.get('used_percent', 0)}% used)"
                                )
                        click.echo()
                    ...
                    if "station" in device_state:
                        ...
                        if station.get("sig_lev") is not None:
                            click.echo(
                                f"Signal Level: {station.get('sig_lev', 'N/A')} dBm"
                            )
                        click.echo()
                    ...
                    if "mount" in device_state:
                        ...
                        click.echo(
                            f"Tracking: {'Yes' if mount.get('tracking') else 'No'}"
                        )
                        click.echo(
                            f"Equatorial Mode: {'Yes' if mount.get('equ_mode') else 'No'}"
                        )
                    ...
                    if "focuser" in device_state:
                        ...
                        click.echo(
                            f"Position: {focuser.get('step', 'N/A')} / {focuser.get('max_step', 'N/A')}"
                        )
                    ...
                    if "balance_sensor" in device_state:
                        ...
                        if "data" in sensor:
                            ...
                            click.echo(
                                f"X: {data.get('x', 'N/A')}, Y: {data.get('y', 'N/A')}, Z: {data.get('z', 'N/A')}"
                            )
                        click.echo()
            ...
            if status_data.get("ra") is not None and status_data.get("dec") is not None:
                click.echo(
                    f"📍 Coordinates: RA={status_data['ra']:.4f}°, Dec={status_data['dec']:.4f}°"
                )
            if status_data.get("alt") is not None and status_data.get("az") is not None:
                click.echo(
                    f"🧭 Alt/Az: Alt={status_data['alt']:.2f}°, Az={status_data['az']:.2f}°"
                )
            ...
            if protocol == "seestar" and client._seestar_client:
                ...
                if seestar_status:
                    ...
                    if seestar_status.dist_deg is not None:
                        click.echo(
                            f"📏 Distance to target: {seestar_status.dist_deg:.2f}°"
                        )
                    if (
                        seestar_status.stacked_frame > 0
                        or seestar_status.dropped_frame > 0
                    ):
                        click.echo(
                            f"📸 Frames: {seestar_status.stacked_frame} stacked, {seestar_status.dropped_frame} dropped"
                        )
                    ...
                    if seestar_status.focus_position is not None:
                        click.echo(
                            f"🔍 Focus Position: {seestar_status.focus_position}"
                        )
                    if (
                        seestar_status.freeMB is not None
                        and seestar_status.totalMB is not None
                    ):
                        used_percent = (
                            (
                                (seestar_status.totalMB - seestar_status.freeMB)
                                / seestar_status.totalMB
                                * 100
                            )
                            if seestar_status.totalMB > 0
                            else 0
                        )
                        click.echo(
                            f"💾 Storage: {seestar_status.freeMB:,} MB free / {seestar_status.totalMB:,} MB total ({used_percent:.1f}% used)"
                        )
...
def park(ctx, host, port):
    ...
    async def park_telescope():
        try:
            client = await create_client(protocol, final_host, final_port)
            click.echo(
                f"🔭 Parking telescope at {final_host}:{final_port} ({protocol})..."
            )
    ...
    async def goto_target():
        try:
            ...
            if success:
                ...
                if status.get("ra") is not None and status.get("dec") is not None:
                    click.echo(
                        f"📍 Current position: RA={status['ra']:.4f}°, Dec={status['dec']:.4f}°"
                    )
            else:
...
@click.option(
    "--duration", "-d", default=10, type=int, help="Stream duration in seconds"
)
@click.pass_context
def stream(ctx, host, port, duration):
    ...
    if not host:
        click.echo(
            "❌ No telescope connection. Use 'connect' command first or provide --host"
        )
        return
        ...
        try:
            ...
            while (asyncio.get_event_loop().time() - start_time) < duration:
                ...
                if status.stacked_frame > 0:
                    click.echo(
                        f"📊 Frames: {status.stacked_frame} stacked, {status.dropped_frame} dropped"
                    )
    ...
    try:
        ...
        if "." in v:
            ...
            if len(parts) >= 2 and parts[0].isdigit() and int(parts[0]) >= 2024:
                ...
                click.echo(
                    f"  Year: {parts[0]}, Month: {parts[1]}, Patch: {parts[2] if len(parts) > 2 else '0'}"
                )
    except Exception:
            ...
            if response and response.result:
                ...
                if "chip_size" in info:
                    ...
                    if isinstance(info["chip_size"], (list, tuple)):
                        click.echo(
                            f"Chip Size: {info['chip_size'][0]}x{info['chip_size'][1]}"
                        )
                    else:
                        click.echo(
                            f"Chip Size: {info['chip_size']['width']}x{info['chip_size']['height']}"
                        )
...
@click.option(
    "--type",
    "-t",
    type=click.Choice(["equ", "radec", "horiz", "all"]),
    default="all",
    help="Coordinate type to get",
)
@click.pass_context
        ...
        try:
            ...
            if type in ["equ", "all"]:
                ...
                if response and response.result:
                    ...
                    if "ra" in coords and "dec" in coords:
                        ...
                        click.echo(
                            f"Equatorial: RA={ra_deg:.4f}°, Dec={coords['dec']:.4f}°"
                        )
            ...
            if type in ["radec", "all"]:
                ...
                if response and response.result:
                    ...
                    if "ra" in coords and "dec" in coords:
                        click.echo(
                            f"RA/Dec: RA={coords['ra']:.4f}, Dec={coords['dec']:.4f}"
                        )
            ...
            if type in ["horiz", "all"]:
                ...
                if response and response.result:
                    ...
                    if "az" in coords and "alt" in coords:
                        click.echo(
                            f"Horizontal: Az={coords['az']:.4f}°, Alt={coords['alt']:.4f}°"
                        )
...
@click.option(
    ...
    "-k",
    multiple=True,
    help="Specific keys to retrieve (e.g., device, pi_status, mount)",
)
@click.option(
    "--all", "show_all", is_flag=True, help="Show all available data without filtering"
)
            ...
            if response and response.result:
                ...
                if "mount" in state:
                    ...
                    if "tracking" in mount:
                        click.echo(
                            f"  Tracking: {'Yes' if mount['tracking'] else 'No'}"
                        )
                    if "equ_mode" in mount:
                        click.echo(
                            f"  Equatorial Mode: {'Yes' if mount['equ_mode'] else 'No'}"
                        )
                ...
                if "storage" in state:
                    ...
                    if "storage_volume" in storage:
                        for vol in storage["storage_volume"]:
                            if "name" in vol:
                                click.echo(
                                    f"  {vol['name']}: {vol.get('freeMB', 0):,} MB free"
                                )
                ...
                if show_all:
                    ...
                    if other_keys:
                        ...
                        click.echo(
                            "  Use --keys to retrieve specific sections or --all to see everything"
                        )
...
@click.option("--port", "-p", type=int, help="Port number")
@click.option(
    "--all", "show_all", is_flag=True, help="Show all settings with raw values"
)
@click.option("--json", "as_json", is_flag=True, help="Output raw JSON data")
@click.pass_context
        ...
        try:
            ...
            if response and response.result:
                ...
                if "auto_power_off" in settings:
                    click.echo(
                        f"  Auto Power Off: {'Enabled' if settings['auto_power_off'] else 'Disabled'}"
                    )
                if "auto_af" in settings:
                    click.echo(
                        f"  Auto Focus: {'Enabled' if settings['auto_af'] else 'Disabled'}"
                    )
                if "auto_3ppa_calib" in settings:
                    click.echo(
                        f"  Auto 3PPA Calibration: {'Enabled' if settings['auto_3ppa_calib'] else 'Disabled'}"
                    )
                ...
                if "stack_lenhance" in settings:
                    click.echo("\n📸 Stack Settings:")
                    click.echo(
                        f"  Light Enhancement: {'Enabled' if settings['stack_lenhance'] else 'Disabled'}"
                    )
                if "stack_after_goto" in settings:
                    click.echo(
                        f"  Stack After Goto: {'Enabled' if settings['stack_after_goto'] else 'Disabled'}"
                    )
                ...
                if "heater_enable" in settings:
                    click.echo("\n🌡️ Heater Settings:")
                    click.echo(
                        f"  Heater: {'Enabled' if settings['heater_enable'] else 'Disabled'}"
                    )
                if "expt_heater_enable" in settings:
                    click.echo(
                        f"  Expert Heater: {'Enabled' if settings['expt_heater_enable'] else 'Disabled'}"
                    )
                ...
                if "factory_focal_pos" in settings:
                    click.echo(
                        f"Factory Focal Position: {settings['factory_focal_pos']}"
                    )
                ...
                if as_json:
                    ...
                    if "View" in state:
                        ...
                        if "lp_filter" in view:
                            click.echo(
                                f"LP Filter: {'Yes' if view['lp_filter'] else 'No'}"
                            )
                        if "lapse_ms" in view:
                        ...
                        if "ra" in state and "dec" in state:
                            click.echo(
                                f"Coordinates: RA={state['ra']:.4f}, Dec={state['dec']:.4f}"
                            )
                        if "mode" in state:
...
@click.option(
    "--output",
    "-o",
    help="Output file path (MP4 format). If not specified, uses temp file",
)
@click.option(
    "--fps",
    type=int,
...
@click.option(
    "--force-rtsp",
    is_flag=True,
    help="Force use of RTSP even if in Stack/ContinuousExposure mode",
)
@click.pass_context
def capture_video(
    ctx, host, port, duration, frames, output, fps, rtsp_port, force_rtsp
):
            ...
            async for image in client.get_next_image(camera_id=0):
                if image and image.image is not None:
                    ...
                    if progress > last_progress:
                        click.echo(
                            f"   Progress: {progress}% ({frames_written}/{target_frames} frames)"
                        )
                        click.echo(
                            f"   Stacked: {client.status.stacked_frame}, Dropped: {client.status.dropped_frame}"
                        )
        ...
        if not force_rtsp:
            ...
            try:
                ...
                if mode in ["Stack", "ContinuousExposure"]:
                    click.echo(
                        f"🔍 Detected {mode} mode - using imaging client for capture"
                    )
                    await capture_from_imaging_client()
                else:
                    click.echo(
                        f"🔍 Detected {mode or 'Idle'} mode - using RTSP stream for capture"
                    )
...
@click.option(
    ...
    "-t",
    type=float,
    help="Timeout in seconds (default: continuous monitoring)",
)
@click.option(
    "--count", "-c", type=int, help="Number of messages to capture before stopping"
)
@click.option("--json", "as_json", is_flag=True, help="Output raw JSON messages")
@click.option(
    "--pretty",
    is_flag=True,
    help="Pretty print JSON messages (default when not using --json)",
)
@click.option(
    "--include",
    "-i",
    multiple=True,
    help="Include only these event types (can be used multiple times)",
)
@click.option(
    "--exclude",
    "-e",
    multiple=True,
    help="Exclude these event types (can be used multiple times)",
)
@click.option(
    "--exec",
    "exec_commands",
    multiple=True,
    help="Execute command after connecting (can be used multiple times)",
)
@click.option(
    "--exec-delay",
    type=float,
    default=1.0,
    help="Delay in seconds before executing commands (default: 1.0)",
)
@click.pass_context
def monitor(
    ctx,
        ...
        try:
            ...
            if command == "status":
                ...
                if len(coords) == 2:
                    ...
                    click.echo("   ❌ Invalid goto format. Use: goto:RA,DEC")
                ...
                response = await client.send_and_recv(
                    MoveFocuser(params=MoveFocuserParameters(step=position))
                )
    ...
    async def monitor_messages():
        ...
        try:
            ...
            if exec_commands:
                click.echo(
                    f"⏳ Waiting {exec_delay} seconds before executing commands..."
                )
                        ...
                        if msg.direction == "received":
                            ...
                            try:
                                ...
                                # Check for response messages with method
                                    ...
                                    event_type = msg_obj.get("method", "").split(".")[
                                        -1
                                    ]  # Get last part of method
                            ...
                            if as_json:
                                ...
                                try:
                                    ...
                                    if "event_type" in parsed_dict:
                                        ...
                                        click.echo(
                                            f"Event: {parsed_dict.get('event_type', 'Unknown')}"
                                        )
                                        if (
                                            "data" in parsed_dict
                                            and parsed_dict["data"]
                                        ):
                                            click.echo("Data:")
                                            import json as json_lib

                                            formatted = json_lib.dumps(
                                                parsed_dict["data"], indent=2
                                            )
                                        ...
                                        click.echo(
                                            f"Method: {parsed_dict.get('method', 'Unknown')}"
                                        )
                                        click.echo(
                                            f"ID: {parsed_dict.get('id', 'N/A')}"
                                        )
                                        if (
                                            "result" in parsed_dict
                                            and parsed_dict["result"]
                                        ):
                                            click.echo("Result:")
                                            import json as json_lib

                                            formatted = json_lib.dumps(
                                                parsed_dict["result"], indent=2
                                            )
                                            for line in formatted.split("\n"):
                                                click.echo(f"  {line}")
                                    else:
...
@click.option("--port", "-p", type=int, help="Port number")
@click.option(
    "--duration", "-d", type=float, help="Duration in seconds (default: continuous)"
)
@click.option(
    "--count", "-c", type=int, help="Number of images to capture before stopping"
)
@click.option("--json", "as_json", is_flag=True, help="Output statistics as JSON")
@click.option(
    "--save",
    "-s",
    help="Directory to save images or video file path (e.g., /tmp/images or /tmp/output.mp4)",
)
@click.option(
    "--save-video",
    is_flag=True,
    help="Save as video file (MP4) instead of individual images",
)
@click.option(
    "--fps",
    type=float,
    default=10.0,
    help="Video frame rate when saving as video (default: 10 fps)",
)
@click.option("--camera-id", type=int, default=0, help="Camera ID (default: 0)")
@click.option(
    "--show-events", is_flag=True, help="Display other telescope events and messages"
)
@click.option(
    "--event-filter",
    multiple=True,
    help="Filter events to show (can be used multiple times)",
)
@click.option("--compact", is_flag=True, help="Use compact one-line display format")
@click.option(
    "--scenery",
    is_flag=True,
    help="Use scenery mode (IscopeStartView) instead of BeginStreaming",
)
@click.option(
    "--verbose-rtsp",
    is_flag=True,
    help="Show RTSP/H.264 decoder warnings (normally suppressed)",
)
@click.pass_context
def stream_images(
    ctx,
    ...
    async def stream_images_task():
        ...
        try:
            ...
            click.echo("🔭 Connecting to telescope...")
            ...
            if scenery:
                ...
                start_view_cmd = IscopeStartView(
                    params=IscopeStartViewParams(
                        mode="scenery", target_name="Scenery Stream"
                    )
                )
            ...
            if not as_json:
                ...
                if compact:
                    click.echo(
                        "Format: [Time] #Num | WxHxC | Size | Δt | FPS | S:stacked D:dropped [pixel info]"
                    )
                click.echo("-" * 60)
            ...
            def display_event(msg, msg_type="EVENT"):
                ...
                try:
                    ...
                    msg_obj = (
                        json_lib.loads(msg.message) if hasattr(msg, "message") else msg
                    )
                    ...
                    if (
                        event_filter_set
                        and event_type
                        and event_type not in event_filter_set
                    ):
                        return
                    ...
                    if compact:
                        ...
                        click.echo(
                            f"[{timestamp}] EVENT: {event_type or 'Unknown'}{event_data}"
                        )
                    else:
                        ...
                        if event_type == "PiStatus" and "Timestamp" in msg_obj:
                            if "battery_capacity" in msg_obj:
                                click.echo(
                                    f"   🔋 Battery: {msg_obj['battery_capacity']}%"
                                )
                            if "temp" in msg_obj:
            ...
            async for image in imaging_client.get_next_image(camera_id=camera_id):
                ...
                if image and image.image is not None:
                    ...
                    if as_json:
                        ...
                        if channels == 1:
                            pixel_info = (
                                f" [{np.min(image.image)}-{np.max(image.image)}]"
                            )
                        else:
                        ...
                        click.echo(
                            f"[{timestamp}] #{images_received:04d} | {width}x{height}x{channels} | {size_mb:.1f}MB | Δt:{ms_since:>7s} | FPS:{fps_str:>5s} | S:{status.stacked_frame:3d} D:{status.dropped_frame:2d}{pixel_info}"
                        )
                    else:
                        # Pretty print output
                        ...
                        click.echo(
                            f"   Size: {bytes_size:,} bytes ({bytes_size / 1024 / 1024:.2f} MB)"
                        )
                        if time_since_last:
                            click.echo(
                                f"   Time since last: {time_since_last * 1000:.1f} ms"
                            )
                            click.echo(f"   Current FPS: {fps:.2f}")
                        click.echo(f"   Average FPS: {avg_fps:.2f}")
                        click.echo(
                            f"   Frames: {status.stacked_frame} stacked, {status.dropped_frame} dropped, {status.skipped_frame} skipped"
                        )

                        # Show image stats
                        if channels == 1:
                            click.echo(
                                f"   Pixel range: [{np.min(image.image)}, {np.max(image.image)}]"
                            )
                        else:
                            for i, color in enumerate(["R", "G", "B"][:channels]):
                                click.echo(
                                    f"   {color} range: [{np.min(image.image[:, :, i])}, {np.max(image.image[:, :, i])}]"
                                )
                    ...
                    if save_path:
                        if save_video:
                            ...
                            if video_writer is None:
                                ...
                                if not as_json:
                                    click.echo(
                                        f"   🎬 Initializing video writer: {vid_width}x{vid_height} @ {vid_fps} fps"
                                    )
                                    click.echo(
                                        f"      Output: {save_path} (codec: {codec_name})"
                                    )
                                ...
                                if not video_writer.isOpened():
                                    ...
                                    for codec, name in fallback_codecs:
                                        if not as_json:
                                            click.echo(
                                                f"   ⚠️ {codec_name} failed, trying {name}..."
                                            )
                                        ...
                                        if video_writer.isOpened():
                                            if not as_json:
                                                click.echo(
                                                    f"   ✅ Video writer opened with {name} codec"
                                                )
                                            break
                                    ...
                                    if not video_writer.isOpened():
                                        ...
                                        click.echo(
                                            f"   Dimensions: {vid_width}x{vid_height}, FPS: {vid_fps}"
                                        )
                                        video_writer = None
                                else:
                                    if not as_json:
                            ...
                            if video_writer:
                                ...
                                if len(image.image.shape) == 3:
                                    bgr_frame = cv2.cvtColor(
                                        image.image.astype(np.uint8), cv2.COLOR_RGB2BGR
                                    )
                                else:
                                    # Grayscale - convert to BGR
                                    bgr_frame = cv2.cvtColor(
                                        image.image.astype(np.uint8), cv2.COLOR_GRAY2BGR
                                    )
                                video_writer.write(bgr_frame)

                                if not as_json and images_received % 10 == 0:
                                    click.echo(
                                        f"   💾 Video: {images_received} frames written"
                                    )
                        else:
            ...
            if video_writer:
                ...
                if not as_json:
                    click.echo(
                        f"✅ Video saved: {save_path} ({images_received} frames)"
                    )
            ...
            if not as_json:
                ...
                if images_received > 0:
                    ...
                    click.echo(f"   Average FPS: {avg_fps:.2f}")
                    click.echo(
                        f"   Final frame counts: {imaging_client.status.stacked_frame} stacked, {imaging_client.status.dropped_frame} dropped"
                    )
                if save_path:
                    if save_video:
    ...
    async def reboot_telescope():
        ...
        try:
            ...
            if response:
                ...
                click.echo(
                    "   The telescope will restart. Wait a few minutes before reconnecting."
                )
            else:
