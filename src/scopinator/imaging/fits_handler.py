...
class FITSHandler:
    ...
    def __init__(self):
        if not HAS_ASTROPY:
            raise ImportError(
                "astropy is required for FITS file handling. Install with: pip install astropy"
            )
        ...
        try:
            with fits.open(file_path) as hdul:
                ...
                if len(image_data.shape) == 3:
                    ...
                    if image_data.shape[0] == 3 and image_data.shape[0] < min(
                        image_data.shape[1], image_data.shape[2]
                    ):
                        # RGB channels first (3, H, W) - transpose to (H, W, 3)
                        image_data = np.transpose(image_data, (1, 2, 0))
                        logging.info(
                            f"Transposed from {original_shape} to {image_data.shape}"
                        )
                    elif image_data.shape[2] == 3 and image_data.shape[2] < min(
                        image_data.shape[0], image_data.shape[1]
                    ):
                        # Already in (H, W, 3) format
                        logging.info(
                            f"Color image in (H, W, 3) format: {image_data.shape}"
                        )
                    elif image_data.shape[0] == 3:
                        # Could be (3, H, W) format
                        image_data = np.transpose(image_data, (1, 2, 0))
                        logging.info(
                            f"Assumed (3, H, W) format, transposed to {image_data.shape}"
                        )
                    else:
                        # Take first channel for non-RGB data
                        logging.info(
                            f"Non-RGB data with shape {original_shape}, taking first channel"
                        )
                        image_data = image_data[0]
                elif len(image_data.shape) > 3:
                    raise ValueError(
                        f"Unsupported image dimensions: {image_data.shape}"
                    )
                ...
                logging.info(
                    f"Loaded FITS file: shape={image_data.shape}, dtype={image_data.dtype}"
                )
                if len(image_data.shape) == 3:
                    logging.info(
                        f"Color image detected with {image_data.shape[2]} channels"
                    )
                else:
    ...
    def normalize_image_data(
        ...
        if is_color:
            ...
            for i in range(3):
                ...
                normalized[:, :, i] = self._normalize_channel(
                    channel, stretch_mode, bg_percent
                )
        else:
            # Process single channel
            single_channel = self._normalize_channel(
                image_data, stretch_mode, bg_percent
            )
            ...
            logging.info(
                f"Converted grayscale to RGB for enhancement compatibility: {normalized.shape}"
            )
    ...
    def fits_to_image(
        ...
        logging.info(
            f"fits_to_image: is_color={is_color}, normalized.shape={normalized.shape}"
        )
        ...
        if is_color:
            ...
            logging.warning(
                f"Unexpected grayscale image in fits_to_image: {image_8bit.shape}"
            )
        ...
        # Prepare metadata
        result_metadata = {
            "dimensions": {"width": image_data.shape[1], "height": image_data.shape[0]},
    ...
    def process_fits_with_enhancements(
        self,
        fits_path: str,
        enhancement_processor: Any,
        output_format: str = "PNG",
        ...
        normalized = self.normalize_image_data(
            image_data, stretch_mode=stretch_mode, bg_percent=bg_percent
        )
        ...
        if is_color:
            ...
            logging.info(
                f"Enhancement: Created RGB PIL image from shape {enhanced_8bit.shape}"
            )
        else:
            # Fallback: if somehow still grayscale, convert to RGB
            logging.warning(
                f"Unexpected grayscale image after enhancement: {enhanced_8bit.shape}"
            )
            if len(enhanced_8bit.shape) == 2:
                enhanced_8bit = np.stack(
                    [enhanced_8bit, enhanced_8bit, enhanced_8bit], axis=2
                )
            pil_image = Image.fromarray(enhanced_8bit, mode="RGB")
            logging.info(
                f"Enhancement: Converted unexpected grayscale to RGB: {enhanced_8bit.shape}"
            )

        # Additional debug info
        logging.info(
            f"Enhancement: PIL image mode: {pil_image.mode}, size: {pil_image.size}"
        )
        ...
        # Prepare metadata
        result_metadata = {
            "dimensions": {"width": enhanced.shape[1], "height": enhanced.shape[0]},
            "original_dimensions": {
